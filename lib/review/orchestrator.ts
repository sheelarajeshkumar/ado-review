 ...
 * review each file with retry, post inline comments, build and post
 * summary. Error isolation ensures one file failure does not stop
 * the pipeline (CORE-05).
 */
...
import { postFileFindings, buildSummaryMarkdown } from './comment-mapper';
 ...
 * 5. Post inline comments for findings
...
export async function runReview(
  ...
  for (let i = 0; i < reviewableChanges.length; i++) {
    const { path: filePath, changeType, change } = reviewableChanges[i];
    ...
    try {
      const fileResult = await retryWithBackoff(
        async () => {
          ...
          // Review with LLM
          const review = await reviewSingleFile(filePath, content, changeType, apiKey);
      ...
      onProgress({
        ...
        payload: {
          ...
          fileIndex: i + 1,
          totalFiles: reviewableChanges.length,
          status: 'posting-comments',
        },
      });

      await postFileFindings(prInfo, filePath, fileResult.findings, iterationId);
      ...
      // Error isolation: record error, continue to next file
  ...
  try {
    const summaryMarkdown = buildSummaryMarkdown(results, prDetails.title);
    await postSummaryComment(prInfo, summaryMarkdown);
  } catch (error) {
    console.error('[PEP Review] Failed to post summary comment:', error);
  }
